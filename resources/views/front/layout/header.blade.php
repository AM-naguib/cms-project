                ...
                <div class="d-flex gap-3 gap-xl-0 align-items-center">
                    ...
                    <div class="logo-default">
                        <a class="navbar-brand text-primary" href="{{ route('front.index') }}">
                            @if (getSettings()->site_logo != null)
                                <img class="img-fluid logo"
                                    src="{{ env('APP_URL') }}/storage/{{ getSettings()->site_logo }}" loading="lazy"
                                    alt="streamit" />
                            @else
                ...
                <nav id="navbar_main"
                    ...
                    <div class="container-fluid p-lg-0">
                            ...
                            <div class="navbar-brand ms-3">
                                ...
                                <div class="logo-default">
                                    <a class="navbar-brand text-primary" href="{{ route('front.index') }}">
                                        @if (getSettings()->site_logo != null)
                                            <img class="img-fluid logo"
                                                src="{{ env('APP_URL') }}/storage/{{ getSettings()->site_logo }}"
                                                loading="lazy" alt="streamit" />
                                        @else
                                            {{ getSettings()->site_title ?? 'Site Title' }}
                                        @endif
                                    </a>
                                </div>
                        ...
                        <ul class="navbar-nav iq-nav-menu  list-unstyled" id="header-menu">
                            ...
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('front.new') }}" role="button" aria-expanded="false"
                                    aria-controls="homePages">
                                    <span class="item-name">المضاف حديثاً</span>
                                </a>
                            </li>
                            @php
                                $mainCategories = \App\Models\MainCategory::with('categories')
                                    ->where('id', '!=', 1)
                                    ->get();
                            @endphp
                            @forelse ($mainCategories as $mainCategory)
                                ...
                                <li class="nav-item">
                                    <a class="nav-link" data-bs-toggle="collapse" href="#features" role="button"
                                        aria-expanded="false" aria-controls="homePages">
                                        <span class="item-name">{{ $mainCategory->name }}</span>
                                    ...
                                    <ul class="sub-nav collapse  list-unstyled" id="features">
                                        @forelse ($mainCategory->categories as $category)
                                            <li><a
                                                    href="{{ route('front.page', ['page' => 'category', 'item' => $category->slug]) }}">{{ $category->name }}</a>
                                            </li>
                                        @empty
                                        @endforelse
