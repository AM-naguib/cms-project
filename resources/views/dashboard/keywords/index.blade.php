        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <button class="btn btn-primary" onclick="showModal('create')">Create Keyword</button>
                        <table id="datatables-reponsive" class="table table-striped w-100">
                            ...
                            <tbody id="tbody">
                                @forelse ($keywords as $keyword)
                                    <tr id="it_{{ $keyword->id }}">
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $keyword->name }}</td>
                                        <td>{{ $keyword->slug }}</td>
                                        <td>
                                            <button type="button" class="btn btn-primary my-1" data-bs-toggle="modal"
                                                onclick="showModal('edit',{{ $keyword->id }})">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </button>

                                            <button class="btn btn-danger" onclick="deleteItem({{ $keyword->id }})"><i
...
@section('js')
    <script>
        ...
        function deleteItem(id) {
            ...
            $.ajax({
                url: "{{ route('dashboard.keywords.destroy', ':id') }}".replace(':id', id),
                type: 'DELETE',
                data: {
                    _token: csrfToken
                },
        ...
        function showModal(type, id = null) {
            if (type === 'create') {
                ...
                let route = "{{ route('dashboard.keywords.store') }}";
                ...
                getItem(id).then(function(result) {
                    ...
                    let route = "{{ route('dashboard.keywords.update', ':id') }}".replace(':id', id);
        ...
        function getItem(id) {
            return $.ajax({
                url: "{{ route('dashboard.keywords.show', ':id') }}".replace(':id', id),
                type: 'GET'
