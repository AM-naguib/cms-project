        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <button class="btn btn-primary" onclick="showModal('create')">Create Genre</button>
                                    ...
                                    <tr id="it_{{ $genre->id }}">
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-primary my-1" data-bs-toggle="modal"
                                                onclick="showModal('edit',{{ $genre->id }})">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </button>

                                            <button class="btn btn-danger" onclick="deleteItem({{ $genre->id }})"><i
...
@section('js')
    <script>
        ...
        function deleteItem(id) {
            ...
            $.ajax({
                url: "{{ route('dashboard.genres.destroy', ':id') }}".replace(':id', id),
                type: 'DELETE',
                data: {
                    _token: csrfToken
                },
        ...
        function showModal(type, id = null) {
            if (type === 'create') {
                $('#centeredModalPrimary .modal-title').text('Create Genre');

                let route = "{{ route('dashboard.genres.store') }}";
                ...
                getItem(id).then(function(result) {
                    ...
                    $('#centeredModalPrimary .modal-title').text('Edit Genre');

                    let route = "{{ route('dashboard.genres.update', ':id') }}".replace(':id', id);
        ...
        function getItem(id) {
            return $.ajax({
                url: "{{ route('dashboard.genres.show', ':id') }}".replace(':id', id),
                type: 'GET'
