        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <button class="btn btn-primary" onclick="showModal('create')">Create Main Category</button>
                                    ...
                                    <tr id="it_{{ $mainCategory->id }}">
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-primary my-1" data-bs-toggle="modal"
                                                onclick="showModal('edit',{{ $mainCategory->id }})">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </button>

                                            <button class="btn btn-danger" onclick="deleteItem({{ $mainCategory->id }})"><i
...
@section('js')
    <script>
        ...
        function deleteItem(id) {
            ...
            $.ajax({
                url: "{{ route('dashboard.main-categories.destroy', ':id') }}".replace(':id', id),
                type: 'DELETE',
                data: {
                    _token: csrfToken
                },
        ...
        function showModal(type, id = null) {
            if (type === 'create') {
                $('#centeredModalPrimary .modal-title').text('Create Main Category');

                let route = "{{ route('dashboard.main-categories.store') }}";
                ...
                getItem(id).then(function(result) {
                    ...
                    $('#centeredModalPrimary .modal-title').text('Edit Main Category');

                    let route = "{{ route('dashboard.main-categories.update', ':id') }}".replace(':id', id);
        ...
        function getItem(id) {
            return $.ajax({
                url: "{{ route('dashboard.main-categories.show', ':id') }}".replace(':id', id),
                type: 'GET'
